...
router.get("/delresturant", function(req, res) {
  if (req.session.rest == true) {
    ...
    db.collection("resturant_details")
      .find({ res_id: req.session.rest_id })
...
router.get("/delete/:id", function(req, res) {
  if (req.session.rest == true) {
    var db = req.app.locals.db;
    db.collection("resturant_details").deleteOne(
...
router.post("/reset_password", function(req, res) {
  ...
  db.collection("rest_owner_details").findOne(
    { id: req.session.rest_id },
    function(err, result) {
      ...
      bcrypt.compare(req.body.current_password, current_pwd, function(
        ...
        if (!pwd_check)
          ...
          bcrypt.hash(req.body.password, saltRounds, function(err, hash) {
            db.collection("rest_owner_details").updateOne(
              { email: result.email },
              { $set: { password: hash } },
...
//     updatedobj.rest_img=req.file.path.substring(15);
      ...
      if (result != null) {
        ...
        bcrypt.genSalt(saltRounds, function(err, salt) {
          bcrypt.hash(req.body.password, salt, function(err, hash) {
            pwd = hash;
            autoIncrement.getNextSequence(db, "rest_owner_details", function(
              err,
              autoIndex
            ) {
              var updatedobj = req.body;
