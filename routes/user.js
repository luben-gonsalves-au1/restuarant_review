      ...
      db.collection("user_reviews")
        ...
        .toArray(function(err, count) {
          db.collection("user_reviews")
            .aggregate([
              { $match: { branch_id: req.params.id } },
              { $group: { _id: "$username", rate: { $avg: "$ratings" } } }
            ])
            .toArray(function(err, avg) {
              db.collection("resturant_details").findOne(
                { res_branch_id: req.params.id },
                function(err, result) {
                  if (avg.length == 1) {
                    avg = avg[0].rate.toFixed(1);
                  }
                  if (req.session.user == true)
                    res.render("user_resturant", {
                      title: "Home",
                      data: result,
...
router.post("/reset_password", function(req, res) {
  ...
  db.collection("user_details").findOne({ id: req.session.user_id }, function(
    err,
    result
  ) {
    var current_pwd = result.password;
    bcrypt.compare(req.body.current_password, current_pwd, function(
      err,
      pwd_check
    ) {
      if (!pwd_check)
        res.render("password_reset", {
          error: "please enter the old password correctly",
          title: "password reset",
          nav: afterlogin
        });
      else {
        bcrypt.hash(req.body.password, saltRounds, function(err, hash) {
          db.collection("user_details").updateOne(
            { email: result.email },
            { $set: { password: hash } },
            function(err, updated) {
              if (err) throw err;
                ...
                req.session.destroy();
                res.render("login", {
                  sucess: "password updated sucessfully",
                  title: "Login",
                  nav: beforelogin
                });
              }
...
router.post("/auth", function(req, res) {
  var db = req.app.locals.db;
  db.collection("user_details").findOne({ email: req.body.email }, function(
    err,
    result
  ) {
    if (result == null)
      res.render("login", {
        error: "Please check the email id or write us cs@mail.com",
        title: "Login",
        nav: beforelogin
      });
    else {
...
router.post("/signup", function(req, res) {
  ...
  db.collection("user_details").findOne({ email: req.body.email }, function(
    ...
    if (result != null) {
      res.render("login", {
        error: "you have already an acount please login",
        title: "login",
        nav: beforelogin
      });
    } else {
      bcrypt.genSalt(saltRounds, function(err, salt) {
        bcrypt.hash(req.body.password, salt, function(err, hash) {
          pwd = hash;
          autoIncrement.getNextSequence(db, "user_details", function(
            err,
            autoIndex
          ) {
            ...
            res.render("login", {
              sucess: "Account created sucessfully please login",
              title: "Login",
              nav: beforelogin
            });
          });
...
router.get("/home", function(req, res) {
  ...
  db.collection("user_reviews")
    .aggregate([
      {
        $group: { _id: "$branch_id", avg: { $avg: "$ratings" } }
      }
    ])
    .toArray(function(err, rating) {
      db.collection("resturant_details")
        .find({})
